...
export default function App() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          try {
            ...
            console.log("Failed to parse payment status response as JSON; cascading to direct client-side Firestore.");
          }
  ...
  const simulatePaymentSuccess = async (customEmail?: string, customStatus?: string, customAmount?: number) => {
    ...
    try {
      ...
      try {
        ...
        console.warn("Failed to parse simulated webhook response as JSON; falling back...");
        data = { success: true, message: "Response parsed as HTML (static host environment fallback applied)." };
      }
      ...
      setSimResponse(`Sukses! Response: ${JSON.stringify(data, null, 2)}`);
    } catch (err: any) {
      console.error("Simulation failed, falling back to direct client write:", err);
      // Fallback on total fetch collapse
      if (targetStatus === "success") {
        const emailLower = targetEmail.toLowerCase().trim();
        localStorage.setItem(`premium_paid_${emailLower}`, "true");
        try {
      ...
      setSimResponse("Simulation fallback (Diregistrasi langsung di Client Firestore): " + (err.message || String(err)));
    } finally {
      setCheckingPayment(false);
    }
  };
  ...
  const handlePayNow = async () => {
    ...
    setError(null);
    const fallbackUrl = `https://lexora.myr.id/m/object-3d-generator-pro/?email=${encodeURIComponent(user.email)}&name=${encodeURIComponent(user.displayName || user.email.split("@")[0])}`;
    try {
      ...
      try {
        ...
        throw new Error("Format response bukan JSON. Mengalihkan ke checkout fallback...");
      }
